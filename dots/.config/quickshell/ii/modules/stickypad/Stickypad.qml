...
Scope {
    ...
    PanelWindow {
        ...
        WlrLayershell.namespace: "quickshell:stickypad"
        WlrLayershell.keyboardFocus: WlrKeyboardFocus.OnDemand
        visible: GlobalStates.stickypadOpen || stickypadBackground.opacity > 0.01
        ...
        property real topOffset: GlobalStates.stickypadOpen ? openMargin : hiddenMargin
        ...
        mask: Region {
            item: stickypadBackground
        }

        StyledRectangularShadow {
            target: stickypadBackground
        }

        Rectangle {
            id: stickypadBackground
            anchors {
                top: parent.top
                right: parent.right
            ...
            opacity: GlobalStates.stickypadOpen ? 1 : 0
            Behavior on opacity {
                NumberAnimation {
                    duration: 120
                    ...
                    StyledTextArea {
                        ...
                        Keys.onPressed: (event) => {
                            if (event.key === Qt.Key_Escape && event.modifiers === Qt.NoModifier) {
                                GlobalStates.stickypadOpen = false
                                event.accepted = true
                                return
                            }
                        }
        ...
        Connections {
            target: GlobalStates
            function onStickypadOpenChanged() {
                if (GlobalStates.stickypadOpen) {
                    pendingReload = true
                    stickypadFile.reload()
    ...
    FileView {
        ...
        onLoaded: {
            ...
            if (pendingReload && GlobalStates.stickypadOpen) {
                pendingReload = false
                Qt.callLater(focusStickypadAtEnd)
            }
    ...
    IpcHandler {
        ...
        function toggle(): void {
            GlobalStates.stickypadOpen = !GlobalStates.stickypadOpen
        }

        function open(): void {
            GlobalStates.stickypadOpen = true
        }
    ...
    GlobalShortcut {
        name: "stickypadToggle"
        description: "Toggles dropdown stickypad"
        onPressed: GlobalStates.stickypadOpen = !GlobalStates.stickypadOpen
    }

    GlobalShortcut {
        name: "stickypadOpen"
        description: "Opens dropdown stickypad"
        onPressed: GlobalStates.stickypadOpen = true
    }

    GlobalShortcut {
        name: "stickypadClose"
        description: "Closes dropdown stickypad"
        onPressed: GlobalStates.stickypadOpen = false
    }
}